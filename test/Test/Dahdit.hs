...
runGetCase getter mayValue bs = do
  let (result, bc, bs') = runGet getter bs
  -- putStrLn ("<<<<" ++ show (result, bc) ++ ">>>>")
  case (result, mayValue) of
    (Left _, Nothing) -> pure ()
    (Left err, Just expec) -> fail ("Got error <" ++ err ++ ">, expected value <" ++ show expec ++ ">")
...
testDahditGet = testGroup "get"
  ...
  , testCase "ByteString" (runGetCase (getByteString 2) (Just (BS.pack [0xEC, 0x5D])) (BS.pack [0xEC, 0x5D, 0xBB]))
  ]
